...
```{r}
mistral_tibble <- read_csv2("corpus_mistral.csv", locale(encoding = "ISO-8859-1"), col_names = TRUE, col_types = NULL)
...
```{r}
grafico_barras <- poemas_palabras_filtrado %>% count(word, sort=TRUE) %>% 
  filter(n > 20) %>% 
